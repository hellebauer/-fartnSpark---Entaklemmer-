...
int  g_iShellyGen  = 2;         // 1=Gen1, 2=Gen2/3     [shared]
...
void handleRunForm(AsyncWebServerRequest *request) {
    ...
    String gen = gp("shellyGen");
    if (gen.length() > 0) { int g = atoi(gen.c_str()); if (g==1||g==2) g_iShellyGen=g; }
    String peak = gp("peak");
...
void getLiveData(AsyncWebServerRequest *request) {
    ...
    pos += snprintf(json + pos, sizeof(json) - pos,
        ...
        g_Body_Data_Site_P_Load, g_ihttpFronius, g_iWifiRSSI,
        (unsigned long)(millis() / 1000)
    );
...
void StoreToEEPROM() {
    ...
    // Shared: PV peak, Shelly generation
...
void ReadSwitchLevelsfromEEPROM() {
    ...
    if (g_iShellyGen != 1 && g_iShellyGen != 2) g_iShellyGen = 2;
...
int shellyCommand(int sw, bool turnOn) {
    ...
    snprintf(url, sizeof(url), "http://%s%s",
        g_sShelly_ip[sw],
        (g_iShellyGen == 1) ? (turnOn ? SHELLY_GEN1_ON : SHELLY_GEN1_OFF)
...
void readShellyPower(int sw) {
    ...
    snprintf(url, sizeof(url), "http://%s%s",
        g_sShelly_ip[sw],
        (g_iShellyGen == 1) ? SHELLY_GEN1_POWER : SHELLY_GEN2_POWER);
    ...
    do {
        if (WiFi.status() == WL_CONNECTED) {
            ...
            if (t1 > t2 + (unsigned long)g_iCycleTime) {
                ...
                // Build Fronius URL
                snprintf(cFronius, sizeof(cFronius), "http://%s%s", g_sWR_ip, FRONIUS_CGI);
                                ...
                                for (int sw = 0; sw < NUM_SW; sw++) {
                                    if (g_bLastStateSent[sw]) {
                                        ...
                                        snprintf(offUrl, sizeof(offUrl), "http://%s%s",
                                            g_sShelly_ip[sw],
                                            (g_iShellyGen == 1) ? SHELLY_GEN1_OFF : SHELLY_GEN2_OFF);
                                        g_bHttpBusy = true;
...
"        </select>\n"
"        <div class=\"hint\">Gilt f&uuml;r alle 4 Shellys</div>\n"
"      </div>\n"
"      <div>\n"
...
"        <div class=\"hint\">Fronius Abfrageintervall. Standard: 3000 ms</div>\n"
"      </div>\n"
"      <div>\n"
